...
--
-- hggplotinternaladvice.md §12 の 4 層 API のうち **Easy 層** (Phase 11 A3)。
-- grammar API (`Hgg.Plot.Spec`) を**そのまま再 export** した上で、
-- 「`inline` を書かずに `[Double]` を直接渡す」 専用ヘルパを足す (= 別名方式)。
--
-- @
-- import Hgg.Plot.Easy
-- import Hgg.Plot.Quick (quickScatter)   -- IO 保存は svg package 側
--
-- -- 1 行で散布図を保存
