...
-- | 長さの単位系 (Phase 33: pt オーサリング + dpi 描画境界)。
--
...
instance FromJSON Length where
  ...
  parseJSON other      = flip (withObject "Length") other $ \o -> do
    ...
    u <- case (uStr :: Text) of
      ...
      "pt" -> pure Pt
      "px" -> pure Px
      _    -> fail ("Hgg.Plot.Unit: unknown LUnit tag " <> T.unpack uStr)
    pure (Length v u)
...
instance FromJSON Pos where
  parseJSON = withObject "Pos" $ \o -> do
    ...
    case (t :: Text) of
      ...
      "npc"    -> PNpc    <$> o .: "p"
      "native" -> PNative <$> o .: "p"
      _        -> fail ("Hgg.Plot.Unit: unknown Pos tag " <> T.unpack t)
