-- |
-- Module      : Hgg.Plot.Spec.Visual
-- Description : VisualSpec (= 外側 Monoid、 図全体の宣言型 spec) + Inset
-- Copyright   : (c) 2026 Aelysce Project (Toshiaki Honda)
-- License     : BSD-3-Clause
--
-- Phase 55: 'Hgg.Plot.Spec' の module 分割で切り出し。 図全体の宣言型 spec
...
import           Hgg.Plot.Unit (Length)
import           Hgg.Plot.Spec.Axis (AxisSpec)
import           Hgg.Plot.Spec.Column (ColRef)
import           Hgg.Plot.Spec.Decoration (Annotation, FontSpec, LegendSpec,
                                               MarginalSpec, ReferenceLine)
import           Hgg.Plot.Spec.Layer (Layer)
import           Hgg.Plot.Spec.Mark (Coord, FacetScales, FacetSpace)
