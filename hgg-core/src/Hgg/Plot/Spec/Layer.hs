-- |
-- Module      : Hgg.Plot.Spec.Layer
-- Description : Layer (内側 Monoid) 本体 + layer-local attribute setter
-- Copyright   : (c) 2026 Aelysce Project (Toshiaki Honda)
-- License     : BSD-3-Clause
--
-- Phase 55: 'Hgg.Plot.Spec' の module 分割で切り出し。 1 layer の全 field を
...
-- mark ごとの構築子は 'Hgg.Plot.Spec.Constructors' 側。 公開 API は従来どおり
-- 'Hgg.Plot.Spec' (facade) が re-export する。 挙動・出力 (JSON 形含む) は
-- 完全に不変。
...
import           Hgg.Plot.Color (Color, fromHexA, fromHexAMaybe, toCss)
import           Hgg.Plot.Unit (Length, lengthToPt)
import           Hgg.Plot.Spec.Column (ColRef)
import           Hgg.Plot.Spec.CustomMark (CustomMark)
