...
data TextStyle = TextStyle
  ...
  , tsFamily :: !Text
  , tsAnchor :: !TextAnchor
  , tsRotate :: !Double         -- degrees CW、 0 = 水平
...
-- ★ raster/vector backend で **唯一の dpi 適用点** ([[Option 1]])。Layout/Render は
...
themePalette t = case t of
  ...
  -- HggCanvas ブランド (panel 塗りあり・grid あり・枠なし、 series は themeSeriesPalette)。
  ...
  -- HggCanvas 正式テーマ (明)。 panel=羊皮紙 cream-light #F8F5EE (canvas 仕様書由来) は据え置き、
  --   外周 plot bg は白 #FFFFFF にして軸内 panel を額装的に強調 (2026-06-02 ユーザ確定)。
  Hgg.Plot.Spec.ThemeCanvas -> ThemePalette
  ...
  Hgg.Plot.Spec.ThemeCanvasDark -> ThemePalette
...
themeStripStyle spec =
  let name = fromMaybe Hgg.Plot.Spec.ThemeDefault (getLast (vsTheme spec))
      ...
      (dbg, dshow) = case name of
        ...
        Hgg.Plot.Spec.ThemeCanvas    -> ("#ece0c8", True)
        Hgg.Plot.Spec.ThemeCanvasDark -> ("#3a3a42", True)
...
tickMarks mSpec layout pal fmtX fmtY rotX rotY showX showY =
  let a  = lpPlotArea layout
      ...
      tsXrot = ts { tsAnchor = AnchorEnd, tsRotate = rotX }
...
shapePath sh (Point cx cy) r = case sh of
  ...
  MShCross ->
    ...
    in [ MoveTo (Point (cx - t) (cy - r))
       ...
       , LineTo (Point (cx - t) (cy - t))
       , ClosePath ]
  -- ★ Phase 35: トランプのスーツ (hgg 独自・ggplot に無い拡張)。 形は
