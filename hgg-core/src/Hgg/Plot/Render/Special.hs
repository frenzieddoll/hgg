-- |
-- Module      : Hgg.Plot.Render.Special
...
module Hgg.Plot.Render.Special where
...
import           Hgg.Plot.Render.EdgeRoute (EdgeRoute (..), routeEdge,
  ...
  Just (DAGSpec nodes es algo plates) ->
    let radius   = doubleOr (lySize ly) (mmPt 11) / 2
        ...
        lookup_ k = case [ n | (i, n) <- nodeMap, i == k ] of
          (n:_) -> Just n
          []    -> Nothing
        obs = dagObstacles toScreen radius nodes nodeMap plates
        ...
        withIx = snd $ foldl'
          ...
          (Map.empty, []) es
        bakeOne (e@(DAGEdge f t _ _), ix) =
          case (lookup_ f, lookup_ t) of
            (Just from, Just to) ->
              let tot = Map.findWithDefault 1 (f, t) parTotal
                  rt  = routeEdge toScreen obs from to (dePath e) radius ix tot
              in e { deRoute = Just (edgeRouteToRouted rt) }
            _ -> e
        es' = map bakeOne withIx
...
renderDAGStandalone area pal ly = case getLast (lyDAG ly) of
  ...
  Just (DAGSpec nodes es algo plates) ->
    ...
    let radius = doubleOr (lySize ly) (mmPt 11) / 2  -- ★ Phase 34 A3: size=直径 統一 (既定 径11mm=半径5.5mm)
        ...
        -- Phase 39 A-1: pt 空間の障害物 (node glyph box + plate box)。 long edge の
        -- box-channel + funnel routing に渡す。
        obs = dagObstacles toScreen radius nodes nodeMap plates
        ...
        edgePrims = concat
          [ renderEdge toScreen obs from to e radius pal ix tot
