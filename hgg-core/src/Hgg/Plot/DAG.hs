...
-- Hgg.Plot.Spec.DAGSpec に変換して描画 layer を作る。 algebraic-graphs
-- (Mokhov) と同じ思想:
--
...
-- @Hgg.Plot.DAG.Typed@) も instance 追加だけで対応。
...
import           Hgg.Plot.DAG.Internal.Sugiyama
                                      (LNode (..), LayoutGraph,
                                       ...
                                       assignOrderFull,
                                       assignRanks, breakCycles,
                                       buildLayoutGraph, lgNodes,
                                       tightenSourceRanks)
import           Hgg.Plot.Layout (dagNodeBaseHalfWidth)
import           Hgg.Plot.Spec   (DAGEdge (..), DAGLayoutAlgorithm (..),
                                      DAGNode (..), DAGNodeKind (..),
...
rankStage raw =
  let nodes = srNodes raw; es = srEdges raw; plates = srPlates raw
      ids = map dnId nodes
      -- P2a (Step 1): back-edge 反転で acyclic 化してから rank 割当。 acyclic 入力
      -- では恒等 (= 現行図に非破壊)。 閉路入力でのみ rank が正しくなる。
      acyclicEs = breakCycles ids [(deFrom e, deTo e) | e <- es]
      -- A2: longest-path / network simplex framework で rank 割当
      lg0 = assignRanks (buildLayoutGraph ids acyclicEs)
      -- Phase 19 A4: source を消費者の直前 rank へ引き締め + エッジ無し plate
      -- メンバを同 plate の最小 rank へ (plate bbox の縦伸び/フローティング解消)
      lg0' = tightenSourceRanks (map dpNodeIds plates) lg0
...
routeStage (StagePositioned raw chainMap rankOf coordMap) =
  let nodes = srNodes raw; es = srEdges raw
      ...
      routedE = [ e { dePath = chainToPath e } | e <- es ]
...
-- 同等処理は 'Hgg.Plot.DAG.Internal.Sugiyama.longestPathRanking' に移動、
