...
> [📚 索引](README.md) ｜ [01 quickstart](01-quickstart.md) ｜ [02 layers](02-layers.md) ｜ [03 encoding & scale](03-encoding-scale.md) ｜ [04 decoration](04-decoration.md) ｜ **05 backends** ｜ [06 dataframe](06-dataframe.md) ｜ [07 analyze](07-analyze.md) ｜ [08 3d](08-3d.md) ｜ [09 appendix](09-appendix.md)
...
| `saveSVGBound path bound` | `IO ()` | `df \|>> spec` で束ねる | **DataFrame**。 列名で書く ([06 dataframe](06-dataframe.md)) |
...
保存関数の選び方を整理する (4 演算子の役割は [演算子早見表](README.md#演算子早見表) が一次根拠)。
**`|>>` だけは保存関数ではなく「データを図に束ねる純関数」**で、 階層が 1 つ上 (`BoundPlot` と
いう値を作るだけ・ファイルは書かない)。 `df |>>` は [06 dataframe](06-dataframe.md) で詳説する。

| | 種類 | データ | 列名検証 | いつ使う |
|---|---|---|---|---|
| `saveSVG path spec` | `IO`(出力) | `inline` を spec に埋込済 | ─ | 値を直接書いた図 |
| `saveSVGWith path r spec` | `IO`(出力) | `Resolver` を手渡し | ─ | 自分で Resolver を持っている |
| `(\|>>)` | **純関数(束ね)** | df を渡すと中で解決 | **する** (`bpDiagnostics`) | df + 列名で書く前段 ([06 dataframe](06-dataframe.md)) |
...
(`width n` / `height n` / `aspectRatio r` / `theme …`、 [04 decoration](04-decoration.md))。 backend は
出力フォーマットの選択だけを担う。
...
> SVG 版 `savePrimitivesSVG` と対称 ([付録 C](09-appendix.md#appendix-extend) の backend 自作)。
> 設定型 `PNGConfig` / `PNGFonts` は PNG backend の構成。 `df |>>` の束ね結果の型クラスは
> `BindableSpec` ([06 dataframe](06-dataframe.md))。
