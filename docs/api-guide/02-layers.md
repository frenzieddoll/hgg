...
> [📚 索引](README.md) ｜ [01 quickstart](01-quickstart.md) ｜ **02 layers** ｜ [03 encoding & scale](03-encoding-scale.md) ｜ [04 decoration](04-decoration.md) ｜ [05 backends](05-backends.md) ｜ [06 dataframe](06-dataframe.md) ｜ [07 analyze](07-analyze.md) ｜ [08 3d](08-3d.md) ｜ [09 appendix](09-appendix.md)

合成単位は **layer**、描画の種類は **mark** (型名 `MarkKind`)、個別関数は `scatter` / `line` / `bar` / …。
mark は `layer (mark <> 修飾子…)` の形で図に重ねる。引数の `ColRef` は **`inline [..]`** (数値)・
**`inlineCat [..]`** (カテゴリ)、または DataFrame 利用時の**列名リテラル** `"weight"` ([06 dataframe](06-dataframe.md))。
...
> scale・軸の制御は [03 encoding & scale](03-encoding-scale.md) にまとめてある。各定型エントリの「encoding」欄は
> そのページの修飾子を指す。
...
> **実例で見る**: 多くの mark の実データ作例は [README ギャラリー](../../README.md#ギャラリー)
> (クリックで各エントリへ) と [R for Data Science 第 1 章](../tutorials/01-visualize/README.md)
...
| `statLm` / `statSmooth` ([07](07-analyze.md)) | `ColRef -> ColRef -> Layer` | 回帰直線 / 平滑 (analyze が fit・帯つき) |
| [`countXY`](#e-countxy) | `ColRef -> ColRef -> Layer` | (x, y) の出現数集計 |
| [`histogramWide`](#e-histogramwide) | `[ColRef] -> VisualSpec` | 複数列を重ねたヒスト |

> 学習モデル由来の stat (`statLm` / `statSmooth` / `statPoly` 等) は [07 analyze](07-analyze.md) を参照。
...
> DataFrame 利用時の例で、`df |>> …` の列を指す ([06 dataframe](06-dataframe.md))。
...
**encoding**
- 必須: x, y (2 つの `ColRef`)
- 任意: [`colorBy`](03-encoding-scale.md#encoding) `colorContinuousBy` `sizeBy` `shapeBy` `alpha` `linetype` (`line`/`step`) `jitterX` / `jitterY` `connect`
...
**関連・ggplot** [`scatterPoints` / `linePoints`](#e-points) (`[Point2]` 直接) ｜ 重なり対策は [`jitterX`](03-encoding-scale.md#encoding) ｜
...
**encoding**
- 必須: x (カテゴリ)・y (高さ)
- 任意: [`colorBy`](03-encoding-scale.md#encoding) `color` `alpha` `position`
...
**encoding** 必須: x, y, ラベル列 ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color` `size` `alpha`
...
**encoding** 必須: x (数値), y ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color` `size` `alpha`
...
**encoding** 必須: 値 1 列 ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color`
...
**encoding** 必須: 値 1 列 ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color` `alpha` `position`
...
**encoding** 必須: 値 1 列 ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color` `alpha`
...
**encoding** 必須: 値 1 列 ／ 任意: [`groupBy`](03-encoding-scale.md#encoding) [`colorBy`](03-encoding-scale.md#encoding) `color` `markWidth`
...
**encoding** 必須: 値 1 列 ／ 任意: [`groupBy`](03-encoding-scale.md#encoding) [`colorBy`](03-encoding-scale.md#encoding) `color` `alpha` `markWidth` `side` `nudge`
...
**encoding** 必須: 値 1 列 ／ 任意: [`groupBy`](03-encoding-scale.md#encoding) [`colorBy`](03-encoding-scale.md#encoding) `nudge`
...
**encoding** 必須: 値 1 列 ／ 任意: [`groupBy`](03-encoding-scale.md#encoding) [`colorBy`](03-encoding-scale.md#encoding) `alpha`
...
**encoding** 各レーン (= 構成 Layer) に [`colorBy`](03-encoding-scale.md#encoding) を載せると、そのレーン内で群ごとに分割 + 彩色
...
**encoding** 必須: x, lo, hi ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color` `alpha`
...
**関連・ggplot** `statLm` ([07](07-analyze.md)・回帰帯) ｜ [`lineRange`](#e-range) (離散区間) ｜
ggplot: `geom_ribbon`
...
**encoding** 必須: x, y (中心), err (対称半幅) ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color`
...
**encoding** 必須: ラベル (カテゴリ), 推定, err (対称半幅) ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color`
...
**encoding** 必須: 推定, SE ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `color`
...
**encoding** 必須: x, y ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) (系列) `alpha`
...
**関連・ggplot** `statLm` ([07](07-analyze.md)・データ由来の回帰) ｜ ggplot: `stat_function`
...
**関連・ggplot** [04 decoration](04-decoration.md) の参照線 ｜ ggplot: `geom_vline(xintercept = mean(x))`
...
**encoding** 必須: カテゴリ, 値 ／ 任意: [`colorBy`](03-encoding-scale.md#encoding)
...
**encoding** 必須: 列リスト ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `alpha`
...
**encoding** 必須: 列リスト ／ 任意: [`colorBy`](03-encoding-scale.md#encoding)
...
**関連・ggplot** 3D 版は [08 3d](08-3d.md) の `quiver3D` ｜ ggplot: `geom_segment(arrow=…)`
...
HBM 抽出子 [`traceOf`](07-analyze.md#hbm-plotting) が自動生成するのが普通。

**encoding** 必須: iter, 値 (`traceLines` は + chain) ／ 任意: [`colorBy`](03-encoding-scale.md#encoding) `alpha`
...
**関連・ggplot** [`ess`](#e-ess) / [`autocorr`](#e-autocorr) ｜ HBM 診断は [07 analyze](07-analyze.md#hbm-plotting) ｜
ArviZ: `plot_trace`
...
**何を描くか** ノードとエッジを直接並べて DAG を描く。HBM からは [`dagOf`](07-analyze.md#hbm-plotting) が
...
**関連・ggplot** HBM 自動生成は [`dagOf`](07-analyze.md#hbm-plotting) ｜ 関連型: `DAGNode` / `DAGEdge` /
`DAGPlate` / `DAGSpec` / `DAGNodeKind` / `DAGLayoutAlgorithm` ｜ `edge :: Text -> Layer` ([encoding](03-encoding-scale.md#encoding)) は別物
