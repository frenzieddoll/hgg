...
> native の概念名は layer / mark)。 3D は別型系 ([08 3d](08-3d.ja.md))。

## ページ一覧

| ページ | 内容 |
|---|---|
| [01 quickstart](01-quickstart.ja.md) | 30 秒で 1 枚 + 書き方の 3 層 (Easy / Grammar / DataFrame) |
| [02 layers](02-layers.ja.md) | レイヤとマーク (描けるグラフ一覧・mark カタログ) |
| [03 encoding & scale](03-encoding-scale.ja.md) | channel (色 / サイズ / 形 / 固定色) + scale / palette + 軸 (position scale) |
| [04 decoration](04-decoration.ja.md) | ラベル / theme / facet / subplot / 座標 / 参照線 / 重畳 / 高度な積層 |
| [05 backends](05-backends.ja.md) | backend (SVG / PDF / PNG / Jupyter) の選び方と保存関数 |
...
2. `<>` は 2 階層。 mark・見た目は **`Layer`** を返し `layer (…)` の**中**、 タイトル・テーマ・facet 等は
   **`VisualSpec`** を返し `layer (…)` の**外**。 → [重畳の仕組み](04-decoration.ja.md#overlay)。
...
| `\|>` | DataFrame の**前方変換** (Hackage `dataframe`・groupBy/aggregate 等) | [06 dataframe](06-dataframe.ja.md) |
| `\|>>` | df を図に**束ねる** (`BoundPlot` を作る純関数・ファイルは書かない) | [06 dataframe](06-dataframe.ja.md#by-column-name) |
| `\|->` | df から列名で**モデルを fit** する | [07 analyze](07-analyze.ja.md#fit-data) |

保存関数 (`saveSVG` / `saveSVGBound` / `saveSVGBoundStats` × SVG/PDF/PNG) は
[05 backends](05-backends.ja.md#be-svg) に一覧がある。
