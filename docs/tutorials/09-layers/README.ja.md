...
**hgg の stat 群分割は color aesthetic で駆動**するので、 `colorBy "drv"` で 3 本に
分けます(色で区別。 honest 記録)。
...
```haskell
mpg |>> theme ThemeGrey <> layer (histogram "hwy" <> binWidth 2)   -- 二峰性・右裾
mpg |>> theme ThemeGrey <> layer (density "hwy")
mpg |>> theme ThemeGrey <> layer (boxplot "hwy")                    -- 外れ値 2 つ
```
