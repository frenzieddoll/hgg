...
main = do
  ...
  saveSVGBoundStats "01-labels.svg" $
    mpg |>> theme ThemeGrey <> layer (scatter "displ" "hwy" <> colorBy "class" <> alpha 0.85)
        <> layer (statSmooth "displ" "hwy" 6 <> color (fromHex "#3366FF"))
        <> palette okabeIto
  ...
  saveSVGBound "02-annotate.svg" $
    mpg |>> theme ThemeGrey <> layer (scatter "displ" "hwy" <> alpha 0.5)
        <> annotate (AnnText { anX = PNative 4.7, anY = PNative 38
                             , anText = "排気量が大きいほど燃費は下がる"
                             , anColor = "#cc3333", anSize = 15 })
  ...
  saveSVGBound "04-palette-shape.svg" $
    mpg |>> theme ThemeGrey <> layer (scatter "displ" "hwy" <> colorBy "drv" <> shapeBy "drv" <> alpha 0.85)
        <> palette tolBright
  ...
  saveSVGBound "05-theme-bw.svg" $
    mpg |>> layer (scatter "displ" "hwy" <> colorBy "class" <> alpha 0.85)
  ...
  saveSVGBoundStats "07-zoom.svg" $
    mpg |>> theme ThemeGrey <> layer (scatter "displ" "hwy" <> colorBy "drv" <> alpha 0.8)
        <> layer (statSmooth "displ" "hwy" 6 <> color (fromHex "#3366FF"))
        <> palette okabeIto
  ...
  saveSVGBound "08-axis-breaks.svg" $
    mpg |>> theme ThemeGrey <> layer (scatter "displ" "hwy" <> colorBy "drv" <> alpha 0.85)
        <> palette okabeIto
