...
文法を手取り足取り学ぶなら [API リファレンス (api-guide/)](api-guide/README.md)、 思想面・matplotlib も含む広い比較は
...
2. **`<>` は 2 階層**。 geom の中の aesthetic (`color` / `size` …) は `layer (geom <> aes)` の **中**、
   theme / scale / facet / labs は **外**。 → [api-guide 03 装飾](api-guide/03-decoration.md)。
3. **`aes(x, y)` のデータは 2 通り**: 値直渡し `scatter (inline xs) (inline ys)` か、
   DataFrame + 列名 `df |>> layer (scatter "x" "y")` ([api-guide 05 dataframe](api-guide/05-dataframe.md))。
...
> [api-guide 03 列挙型早見表](api-guide/03-decoration.md#enum-tables)。
...
描画は stat-in 経路の `saveSVGBoundStats` ([api-guide 06 analyze (ルート 2 stat-in)](api-guide/06-analyze.md))。
`statLm` を使わず fit 済みモデルを重ねるなら `toPlot (lmModel x y)` (ルート 1、 同 06 analyze)。
