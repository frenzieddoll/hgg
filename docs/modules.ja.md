...
| モジュール | 責務 |
|---|---|
| `Graphics.Hgg.Spec` | 図の宣言型仕様 (Grammar API)。 geom / channel / scale / theme / facet |
| `Graphics.Hgg.Easy` | 入門 API。 Spec を再 export + `[Double]` 直渡しヘルパ + `overlay` |
| `Graphics.Hgg.Layout` | domain 計算・track 配置・facet レイアウト |
| `Graphics.Hgg.Render` | `Primitive` (幾何プリミティブ) と backend 抽象 |
| `Graphics.Hgg.Palette` | Hgg ブランド + 学術 palette、 sequential/diverging/cyclical |
| `Graphics.Hgg.DAG` | グラフ図 (graphviz dot 相当、 Sugiyama 配置) |
| `Graphics.Hgg.Validate` | spec の妥当性検査 |
