...
--   * 'saveHTML3D'  ─ Phase 4 WebGL bundle 埋込 HTML (= 'Hgg.Plot.ThreeD.Browser')
--   * 'showBrowser' ─ tmp HTML + xdg-open (= 'Hgg.Plot.ThreeD.Browser')
--
...
import           Hgg.Plot.Backend.PDF         (savePrimitivesPDF)
import           Hgg.Plot.Backend.Rasterific  (savePrimitivesPNG, defaultPNGConfig)
import           Hgg.Plot.Render              (FillStyle (..), Point (..),
                                                   Primitive (..), solid,
                                                   StrokeStyle (..),
                                                   TextAnchor (..),
                                                   TextStyle (..))
import           Hgg.Plot.Render.Common       (continuousColor)
import           Hgg.Plot.Math.Griddata       (innerLevels, marchingSegments)
import           Hgg.Plot.Layout              (Rect (..))

import           Hgg.Plot.ThreeD.Axes         (Axes3D (..),
                                                    defaultPaneStyle3D,
                                                    renderAxes3DPanes,
                                                    renderAxes3DWithLabels)
import           Hgg.Plot.ThreeD.Line         (renderLine3D, renderWireframe3D,
                                                    renderQuiver3D)
import           Hgg.Plot.ThreeD.Projection   (Projected (..), Viewport (..),
                                                    project3D)
import           Hgg.Plot.ThreeD.Scatter      (scatterPointsDepth)
import           Hgg.Plot.ThreeD.Spec         (Layer3D, Mark3DKind (..),
                                                    VisualSpec3D (..),
...
import           Hgg.Plot.ThreeD.Surface      (surfaceFacesDepth,
                                                    trianglesFacesDepth)
import           Hgg.Plot.ThreeD.Delaunay     (delaunay2D)
import           Hgg.Plot.ThreeD.Bar          (Bar3D (..), barFacesDepth,
