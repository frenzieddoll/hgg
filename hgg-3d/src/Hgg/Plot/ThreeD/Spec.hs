-- |
-- Module      : Hgg.Plot.ThreeD.Spec
-- Description : 3D VisualSpec / Layer + Monoid (Phase 5 A3)
-- Copyright   : (c) 2026 Hgg
-- License     : BSD-3-Clause
--
-- 2D 同型 (= Hgg.Plot.Spec の 'VisualSpec' / 'Layer' / Monoid 構成) を踏襲した
...
import           Hgg.Plot.ThreeD.Axes       (Axes3D (..), defaultAxes3D)
import           Hgg.Plot.ThreeD.Line       (Line3D (..), Wireframe3D (..),
                                                 defaultLine3D, defaultWireframe3D,
                                                 Quiver3D (..), defaultQuiver3D)
import           Hgg.Plot.ThreeD.Scatter    (Scatter3D (..), defaultScatter3D)
import           Hgg.Plot.ThreeD.Surface    (Surface3D (..), defaultSurface3D)
import           Hgg.Plot.ThreeD.Bar        (Bar3D (..), BarStyle3D (..),
                                                 defaultBar3D)
import           Hgg.Plot.ThreeD.Delaunay   (delaunay2D)
import           Hgg.Plot.Spec              (ColRef, Resolver, resolveNum, resolveCol,
                                                 ColData (..))
import           Hgg.Plot.Color             (Color, toCss, fromHexA, fromHexAMaybe)
...
-- | 3D scatter (= M3Scatter)。 Phase 24 A6: 2D 'Hgg.Plot.Spec.scatter' と
...
-- | Phase 30 A6: 列駆動の 3D テキスト注釈 (= 2D 'Hgg.Plot.Spec.text' と対称・
...
-- | Phase 30 A5: 固定色 (= 2D 'Hgg.Plot.Spec.color' 同型・型安全な 'Color')。
...
-- | 便利関数 (2D 'Hgg.Plot.Spec.colorRGBA' の 3D 双子): 8 桁 RGBA hex
...
layerToLine l =
  ...
  in base
       { Hgg.Plot.ThreeD.Line.l3Color = fromMaybe (Hgg.Plot.ThreeD.Line.l3Color base) (getLast (lyr3Color l))
       , Hgg.Plot.ThreeD.Line.l3Width = fromMaybe (Hgg.Plot.ThreeD.Line.l3Width base) (getLast (lyr3Width l))
       }
