...
import           Hgg.Plot.Spec              (ColRef (..), Resolver)
import           Hgg.Plot.Validate          (DiagnosticContext (..),
                                                 PlotDiagnostic (..),
                                                 PlotErrorKind (..), suggest)
import           Hgg.Plot.Frame             (BindableSpec (..),
                                                 PlotData (..),
                                                 emptyDfDiagnostics)

import           Hgg.Plot.ThreeD.Easy       (saveSVG3D)
import           Hgg.Plot.ThreeD.Browser    (saveHTML3D, showBrowser)
import           Hgg.Plot.ThreeD.Spec       (Layer3D, VisualSpec3D (..),
                                                 lyr3EncX, lyr3EncY, lyr3EncZ,
                                                 lyr3TextBy, resolveSpec3D)

-- | df バインド済の 3D plot (2D 'Hgg.Plot.Frame.BoundPlot' の 3D 版)。
