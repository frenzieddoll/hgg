...
```sh
cd docs/tutorials/07-communication
cabal run tut-07-communication
```
...
| `aes(color = drv, shape = drv)` | `color "drv" <> shapeBy "drv"` |
...
前章 → [`06-eda`](../06-eda/)。
次章 → [`08-datetimes`](../08-datetimes/)(Ch17 Dates and times・flights)。
