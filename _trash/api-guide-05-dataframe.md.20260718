...
> [📚 索引](README.md) ｜ [01 quickstart](01-quickstart.md) ｜ [02 layers](02-layers.md) ｜ [03 decoration](03-decoration.md) ｜ [04 backends](04-backends.md) ｜ **05 dataframe** ｜ [06 analyze](06-analyze.md) ｜ [07 3d](07-3d.md) ｜ [08 appendix](08-appendix.md)
...
### df を用意する 3 つの方法 (`class PlotData`)
...
df |>> ( layer (scatter "x" "y" <> color "group" <> sizeBy "size" <> alpha 0.85)
       <> title "df |>> scatter (color/size を列名で)" )
```
![df scatter](images/df/01-scatter-color-size.svg)

```haskell
-- (b) 重畳: 同じ df の別 layer を <> (03 decoration の重畳と同じ)
df |>> ( layer (scatter "x" "y" <> size 6)
       <> layer (line "x" "y" <> colorStatic "#d62728" <> stroke 1) )
```
![df overlay](images/df/02-overlay.svg)

```haskell
-- (c) facet: 列で小分け (free/fixed scale も VisualSpec で指定可)
df |>> ( layer (scatter "x" "y" <> color "group" <> size 6) <> facet "group" )
```
![df facet](images/df/03-facet.svg)

```haskell
-- (d) 棒グラフ: カテゴリ列 + 群 + position adjustment
dfB |>> ( layer (bar "cat" "val" <> color "grp" <> position PosDodge) )
```
![df bar dodge](images/df/04-bar-dodge.svg)

同様に `boxplot "y"` / `histogram "x"` / `violin "g" "y"` / `heatmap "c" "r" "v"` …
[02 layers](02-layers.md#marks) の mark はすべて列名引数で df から描ける。 実際に動く全例は
`cabal run df-plot-demo` (`hgg-svg/examples/DfPlotDemo.hs`)。

### Hackage `dataframe` (CSV 等)
...
main = do
  ...
  saveSVGBound "out.svg" (df |>> layer (scatter "weight" "mpg" <> color "origin"))
```
...
> (`filter |> ggplot` と同型・[3f-2 subplot](#subplots) の patchwork と組み合わせる)。
