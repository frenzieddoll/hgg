...
> [📚 索引](README.md) ｜ [01 quickstart](01-quickstart.md) ｜ [02 layers](02-layers.md) ｜ [03 decoration](03-decoration.md) ｜ **04 backends** ｜ [05 dataframe](05-dataframe.md) ｜ [06 analyze](06-analyze.md) ｜ [07 3d](07-3d.md) ｜ [08 appendix](08-appendix.md)
...
| `saveSVGBound path bound` | `IO ()` | `df \|>> spec` で束ねる | **DataFrame**。 列名で書く ([05 dataframe](05-dataframe.md)) |
...
混同しやすいので整理する。 **`|>>` だけは保存関数ではなく「データを図に束ねる純関数」**
で、 階層が 1 つ上 (`BoundPlot` という値を作るだけ・ファイルは書かない)。 `df |>>` は
[05 dataframe](05-dataframe.md) で詳説する。

| | 種類 | データ | 列名検証 | いつ使う |
|---|---|---|---|---|
| `saveSVG path spec` | `IO`(出力) | `inline` を spec に埋込済 | ─ | 値を直接書いた図 |
| `saveSVGWith path r spec` | `IO`(出力) | `Resolver` を手渡し | ─ | 自分で Resolver を持っている |
| `(\|>>)` | **純関数(束ね)** | df を渡すと中で解決 | **する** (`bpDiagnostics`) | df + 列名で書く前段 ([05 dataframe](05-dataframe.md)) |
| `saveSVGBound path bound` | `IO`(出力) | `\|>>` が作った束を消費 | (束に同梱済を報告) | `\|>>` の出力 |

```haskell
-- DataFrame ルート (推奨): |>> で束ねて saveSVGBound で出す (05 dataframe)
...
(`width n` / `height n` / `aspectRatio r` / `theme …`、 [03 decoration](03-decoration.md))。 backend は
出力フォーマットの選択だけを担う。

### Jupyter inline ─ `hgg-ihaskell` (Experimental)
...
セル値にする (`instance IHaskellDisplay BoundPlot`)。 旧 `DisplayPlot` は deprecated。

### PDF ─ `savePDF` (Phase 17・HPDF)
...
### PNG ─ `savePNG` (Phase 22・Rasterific・日本語可)
